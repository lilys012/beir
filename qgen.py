...
def qgen(data_path, output_dir, first_token=None, generator_name_or_path='BeIR/query-gen-msmarco-t5-base-v1', ques_per_passage=3, bsz=32, qgen_prefix='qgen'):
    #### Provide the data_path where nfcorpus has been downloaded and unzipped
    corpus = GenericDataLoader(data_path).load_corpus()

    #### question-generation model loading 
    generator = QGen(model=QGenModel(generator_name_or_path, first_token))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--first_token', required=False, default=None)
    args = parser.parse_args()
    qgen(args.data_path, args.output_dir, args.first_token)
    corpus, gen_queries, gen_qrels = GenericDataLoader(args.output_dir, prefix='qgen').load(split="train")
    print(len(gen_queries))
    print(gen_queries)
